...
def spawn_carrot():
    ...
    x = random.randint(0,WIDTH - w)
    y = HEIGHT - player_duck.get_height() - 60
...
def apple_collision():
    ...
    for i in reversed (range(len(apples))):
        ...
        if player_rect.colliderect(apple_rect) or time> apple[0]+3000 :
            del apples [i]
    for i in reversed (range (len(carrots))):
        ...
        if player_rect.colliderect(carrot_rect) or time> carrot[0]+3000:
            del carrots [i]
...
def draw():
    ...
    pygame.display.flip()
def update():
    global player_x , movement_speed, grounded, player_y, acc, jumping, jump_speed, time
